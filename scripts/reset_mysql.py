...
try:
        ...
        table = "Food_Water"
        sql = ""
        sql = "CREATE TABLE " + table + "(ID INT(10) PRIMARY KEY AUTO_INCREMENT, NAME_PET VARCHAR(255) NOT NULL, FOOD INT(15) NOT NULL, WATER INT(15) NOT NULL, TIME DATETIME NOT NULL)"
        print "Running: %s...." %sql,
        try:
                cursor.execute(sql)
                db.commit()
