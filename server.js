...
var options1 = {
    ...
    pythonOptions: ['-u'],
    scriptPath: '/home/udooer/APFS_Project/pages/python/Neo.GPIO/'
};
...
io.sockets.on("connection", function (socket) {
	...
	console.log("stop alarm");
		PythonShell.run('AlarmOn.py', options1, function (err, results) {
			if (err) throw err;
			console.log("Alarm");
		});
    });
	...
	console.log(data);
	if(data.id == "switch1"){
		console.log("Feeding for Pets");
		PythonShell.run('FoodOn.py',options2,function (err, results){
			if (err) throw err;
			console.log("Finish Feeding");
		});
	}
	if(data.id == "switch2"){
		console.log("Drink for Pets");
		PythonShell.run('WaterOn.py',options3, function (err, results){
			if (err) throw err;
	...
	if(data.id == "switch3"){
		console.log("Lamp On");
		PythonShell.run('LampOn.py',options4, function (err, results){
			if (err) throw err;
			console.log("Finsish Lamp");
		});
	}
   });
